...
- If you lose/gain a bounded [number]{.alert} of data, i.e., $n = n_m \pm n_0$, then $\Dsc\bigl(\{\vx_i\}_{i=0}^{n-1}\bigr) = \Order\bigl(n_m^{-\min(p,1)}\bigr)$ 
...
- Don't sweat it if you lose a few or have a few extra
  - If you really want higher order convergence, then use reproducing/covariance kernels to find optimalsample weights, but this takes work
