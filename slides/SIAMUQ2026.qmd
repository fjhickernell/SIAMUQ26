...
subtitle: |
  ...
  Joint work with Claude Hall, Jr., Jimmy Nguyen, Alexander Pride, Aleksei G. Sorokin, and <br>
  the <span class="pkg">qmcpy</span> development team
...
\\ \le \underbracket{\Dsc\bigl(\{\vx_i\}_{i=0}^{n-1}\bigr)}_{\Order(\alert{n^{-1+\delta}}) \text{ or \class{alert}{better}}} \, \lVert f \rVert_{\cf}
\end{multline*}
...
Suppose $\Dsc\bigl(\{\vx_i\}_{i=0}^{n_k-1}\bigr) \le C n_k^{-p}$ for a sequence of preferred sample sizes, $n_1, n_2, \ldots$: 

- If $p > 1$, then $n_{k+1} - n_k$ cannot be bounded

- If you lose/gain a bounded [number]{.alert} of data, i.e., $n = n_k \pm n_0$, then $\Dsc\bigl(\{\vx_i\}_{i=0}^{n-1}\bigr) = \Order(n_k^{-\min(p,1)})$ 

  - If $p \le 1$, then you do not lose any thing asymptotically

  - If $p > 1$, then you still have $\Order(n_k^{-1})$ convergence
...
\begin{align*}
\Dsc\bigl(\{\vx_i\}_{i=0}^{n-1}, \cf \bigr) &:= \sup_{\lVert f \rVert_{\cf} \le 1 }\left \lvert \int_{[0,1]^d} f(\vx) \, \dif \vx - \frac 1n \sum_{i=0}^{n-1} f(\vx_i) \right \rvert \\
 & = 
...
\WESD (N,d) := \sum_{n=1}^N \class{alert}{n} \, \ESD \bigl (\{\vx_i\}_{i=0}^{n-1}, \cf \bigr)
\end{gather*}
...
\begin{align*}
\textrm{ESD}\bigl (\{\phi_b(i) \vzeta \}_{i=0}^{n-1} \ \class{alert}{\text{lattice}}, \cf \bigr)
& = - \overline{K} + \frac 1{n^2} \biggl [ n \tK(\boldsymbol{0}) +  \sum_{k=1}^{b^{\lceil \log_b n\rceil}-1} \#_n(k) \tK\bigl( \phi_b(k) \boldsymbol{\zeta}\bigr) \biggr] \\
\textrm{ESD}\bigl (\{i \vzeta \}_{i=0}^{n-1}\ \class{alert}{\text{Kronecker}}, \cf \bigr) 
& = - \overline{K} + \frac 1{n^2} \biggl [ n \tK(\boldsymbol{0}) + 2 \sum_{k=1}^{n -1}(n - k) \tK( k \boldsymbol{\zeta}) \biggr] \\
...
\WESD (N,d)  & := \sum_{n=1}^N \class{alert}{n} \, \ESD \left (\begin{Bmatrix} \{\phi_b(i) \vzeta \}_{i=0}^{n-1} \text{ lattice} \\ \{i \vzeta \}_{i=0}^{n-1} \text{ Kronecker} \end{Bmatrix}, \cf \right) \\ 
& \qquad \qquad \text{in } \class{alert}{\Order(N)} \text{ operations}
\end{align*}


## Component-by-component (CBC) constructions for the generating vector $\vzeta$ \\ assuming decaying coordinate weights $\gamma_\ell = \ell^{-2}$, $\ell=1, \ldots, d$
...
<h3>Conclusion</h3> 
- Keep the preferred nodes, $\{\vx_i\}_{i=0}^{n_k}$, optimal low discrepancy
- Don't sweat it if you lose a few or have a few extra
  - If you really want higher order convergence, then use reproducing/covariance kernels to find sample weights
...
$$
c n_k^{-P} \le \Dsc\bigl(\{\vx_i\}_{i=0}^{n_k-1}\bigr) \le C n_k^{-p} \qquad k = 1, 2, \ldots
$$
...
## $n_{k+1} -  n_k$ must grow if $p > 1$

If $n_- = n_{k}$, $n_+ = n_{k+1}$, and $n_0 = n_{k+1} - n_k$, then 

\begin{align*}
0 < a (n_{k+1} - n_k) ^{-Q} & \le \Dsc\bigl(\{\vx_i\}_{i=n_k}^{n_{k+1}-1}\bigr) \\
& \le \frac{n_{k+1}}{n_{k+1} - n_k} \Dsc\bigl(\{\vx_i\}_{i=0}^{n_{k+1}-1}\bigr) + \frac{n_k}{n_{k+1} - n_k} \Dsc\bigl(\{\vx_i\}_{i=0}^{n_{k}-1}\bigr) \\
& \le (n_{k+1} - n_k)^{-1} C (n_{k+1}^{1-p} + n_k^{1-p}) \\
a &\le C (n_{k+1} - n_k)^{Q-1} (n_{k+1}^{1-p} + n_k^{1-p}) \\
\end{align*}

So if $1 < p <Q$, then $n_{k+1} - n_k$ must grow at least on the order of $n_k^{\frac{p-1}{Q-1}}$

## $\Order\bigl(n_k^{-\min(p,1)}\bigr)$ discrepancy if we lose/gain a bounded number of data

### Losing points

If $n_{+} = n_k$, and $n_- = n_k - n_0$ then 

\begin{align*}
\Dsc\bigl(\{\vx_i\}_{i=n_0}^{n_{k} -n_0 -1}\bigr)
& \le \frac{n_{k}}{n_{k} - n_0} \Dsc\bigl(\{\vx_i\}_{i=0}^{n_{k}-1}\bigr) + \frac{n_0}{n_{k} - n_0} \Dsc\bigl(\{\vx_i\}_{i=n_k - n_0}^{n_k-1}\bigr) \\
& \le  \frac{C n_{k}^{-p} + An_0 n_k^{-1}}{1 - n_0/n_k} 
\end{align*}
...
\begin{align*}
\Dsc\bigl(\{\vx_i\}_{i=n_0}^{n_{k} + n_0 -1}\bigr)
& \le \frac{n_{k}}{n_{k} + n_0} \Dsc\bigl(\{\vx_i\}_{i=0}^{n_{k}-1}\bigr) + \frac{n_0}{n_{k} + n_0} \Dsc\bigl(\{\vx_i\}_{i=n_k}^{n_k+n_0-1}\bigr) \\
& \le  \frac{C n_{k}^{-p} + An_0 n_k^{-1}}{1 + n_0/n_k} 
\end{align*}
...
- If $n_0$ is bounded, then the discrepancy decays as $\Order(n_k^{-\min(p,1)})$ as $k\to\infty$

- If instead $n_0 \le \alpha n_k$, then the discrepancy is no worse than proportional to $\alpha$, i.e.
