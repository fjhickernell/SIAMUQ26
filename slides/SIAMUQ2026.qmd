...
\Dsc^2\bigl (\{\vx_i\}_{i=0}^{n-1}, \cf \bigr) & = 
...
$$
\ESD \bigl (\{\vx_i\}_{i=0}^{n-1}, \cf \bigr) := \Ex\left[ \Dsc^2\bigl (\{\vx_i\}_{i=0}^{n-1}, \cf \bigr) \right] \qquad 
...
\WESD (N,d)  & := \sum_{n=1}^N \class{alert}{n} \, \ESD \left (\begin{Bmatrix} \{\phi_b(i) \vzeta \}_{i=0}^{n-1} \text{ lattice} \\ \{i \vzeta \}_{i=0}^{n-1} \text{ Kronecker} \end{Bmatrix}, \cf \right) \qquad \text{in } \class{alert}{\Order(N)} \text{ operations}
\end{align*}


## Component-by-component (CBC) constructions w/ decaying coordinate weights $\gamma_\ell = \ell^{-2}$, $\ell=1, \ldots, d$
